...
export const getProfitMaxTempItem = async (key: string) => {
  const data = await readDataJson();
  let item: any;
  if ("tempProfitMaxiModeItem" in data[key])
    item = data[key]["tempProfitMaxiModeItem"];
  else {
  ...
  if (item?.address)
    content.push([{ text: "Add to list", callback_data: "Temp_To_List" }]);
...
export const addTempToList = async (userId: string) => {
  ...
  if (tempItem.address && tempItem.name && tempItem.symbol) {
    const isDuplicate = data[userId]["profitMaxList"].some(
      (item: any) => item.address === tempItem.address
    );
    if (isDuplicate) {
      return {
        success: false,
        message: `Address <code>${tempItem.address}</code> already exists.`,
      };
    }
    ...
    return { success: false, message: `No temporarily data ready` };
  }
};
...
export const addProfitMaxAddress = async (
  ...
  if ("profitMaxList" in data[key]) {
    ...
    if (isDuplicate) {
      return {
        success: false,
        message: `Address <code>${address}</code> already exists.`,
      };
    }
  }
  data[key]["tempProfitMaxiModeItem"] = {
...
export const addProfitMaxPrice = (
  ...
  return {
    success: true,
    message: `Price <code>${price}</code> has been added.`,
  };
};
