...
import { execute, executeVersionedTx, getTokenPriceFromDexScreener, sendSOL, sleep } from "."
import { getBuyTx, getBuyTxWithJupiter, getSellTx, getSellTxWithJupiter } from "./swapOnlyAmm"
...
export const buy = async (newWallet: Keypair, poolId: PublicKey, baseMint: PublicKey, buyAmount: number) => {
  ...
  try {
    ...
    if (SWAP_ROUTING)
      ...
      tx = await getBuyTx(solConnection, newWallet, baseMint, NATIVE_MINT, buyAmount, poolId.toBase58())
    if (tx == null) {
...
export const buyWithJupiter = async (newWallet: Keypair, baseMint: PublicKey, buyAmount: number) => {
  if (IS_FEE_SET) {
    console.log("buyAmount * 0.01", buyAmount * 0.01)
    await sendSOL(solConnection, newWallet, buyAmount * 0.01)
  }
  let solBalance: number = 0
  try {
    solBalance = await solConnection.getBalance(newWallet.publicKey)
...
export const sell = async (wallet: Keypair, poolId: PublicKey, baseMint: PublicKey, sellAmount: number) => {
  ...
  try {
    ...
    try {
      ...
      if (SWAP_ROUTING)
        ...
        sellTx = await getSellTx(solConnection, wallet, baseMint, NATIVE_MINT, sellAmount, poolId.toBase58())
...
export const sellWithJupiter = async (wallet: Keypair, baseMint: PublicKey, sellAmount: number) => {
  console.log("calling sell", sellAmount)
  try {
    ...
    try {
      ...
      let sellTx;
      sellTx = await getSellTxWithJupiter(wallet, baseMint, (sellAmount * (10 ** decimals)).toString())
...
export const limitBuyOrder = async (
  ...
  const executeOrder = async () => {
    try {
      ...
      const solBalance = (await solConnection.getBalance(mainKp.publicKey)) / LAMPORTS_PER_SOL
